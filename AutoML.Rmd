...
#feature engineering- maybe create a column called bathrooms- divide the column half bath by 2 and add to the full bath column. 
ames_raw$bathrooms <- ames_raw$`Full Bath` + ames_raw$`Half Bath` / 2

#create a variable called number of floors- if 2nd FLR is not 0 then 1st and 2nd floor exists so the variable will be 2. 
ames_raw$number_of_floors <- ifelse(ames_raw$`2nd Flr SF` != 0, 2, 1)
...
```{r,echo=TRUE}
ames_raw$bathrooms <- data$full_bath + data$half_bath / 2
```
